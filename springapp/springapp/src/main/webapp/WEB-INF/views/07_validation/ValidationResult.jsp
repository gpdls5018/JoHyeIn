...
<jsp:include page="/WEB-INF/views/template/Top.jsp"/>
    <div class="container" style="margin-top: 49px">
        <div class="jumbotron">
            <h1>Spring Framework <small>ValidationResult</small></h1>           
        </div><!--jumbotron-->
        <fieldset class="form-group border p-3">
        	<legend class="w-auto px-3">유효성 검증완료</legend>	
				...
				<div class="form-group">
					...
					<div class="d-flex">
						<div class="custom-control custom-radio mr-2">
							<input type="radio" class="custom-control-input" name="gender" <c:if test="${param.gender=='남자'}">checked</c:if> value="남자" id="male1"> 
								<label for="male1" class="custom-control-label">남자</label>
						</div>
						<div class="custom-control custom-radio">
							<input type="radio" class="custom-control-input" name="gender" <c:if test="${param.gender=='여자'}">checked</c:if> value="여자" id="female1"> 
				...
				<div class="form-group">
					...
					<div class="d-flex">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" class="custom-control-input" name="inter" <c:if test="${fn:contains(fn:join(inter,','),'정')}">checked</c:if> value="정치" id="POL1"> 
							<label class="custom-control-label" for="POL1">정치</label>
						</div>
						<div class="custom-control custom-checkbox mx-2">
							<input type="checkbox" class="custom-control-input" name="inter" <c:if test="${fn:contains(fn:join(inter,','),'경')}">checked</c:if> value="경제" id="ECO1"> 
							<label class="custom-control-label" for="ECO1">경제</label>
						</div>
						<div class="custom-control custom-checkbox">
							<input type="checkbox" class="custom-control-input" name="inter" <c:if test="${fn:contains(fn:join(inter,','),'연')}">checked</c:if> value="연예" id="ENT1"> 
							<label class="custom-control-label" for="ENT1">연예</label>
						</div>
						<div class="custom-control custom-checkbox ml-2">
							<input type="checkbox" class="custom-control-input" name="inter" <c:if test="${fn:contains(fn:join(inter,','),'스')}">checked</c:if> value="스포츠" id="SPO1"> 
